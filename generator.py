...
def code128_image(data, height=100, thickness=3, quiet_zone=True): #, label=False, font_size=20):
    input_string = data
    bar_height = height
    # if label:
    #     bar_height = height - 1.5 * font_size 
    ...
    total_width = sum(barcode_widths)
    x = 0

    if quiet_zone:
        total_width += 20 * thickness
        x = 10 * thickness
    ...
    for width in barcode_widths:
        if draw_bar:
            draw.rectangle(((x, 0), (x + width - 1, bar_height)), fill=0)
        draw_bar = not draw_bar
        x += width

    # Draw label under the barcode
    # if label:
    #     font = ImageFont.truetype("/System/Library/Fonts/AppleGothic.ttf", font_size)
    #     w, h = draw.textsize(input_string, font=font)
    #     draw.text((total_width / 2 - w / 2, height - h + 1), input_string, font=font, fill="black")
